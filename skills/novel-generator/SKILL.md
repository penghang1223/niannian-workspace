...
## 📦 工作流 v4.0 集成规则（2026-04-09 强制写入）

### 飞书文档规范
1. **先建项目子目录，再建文档**——不能散落在根目录
2. 文档命名：`{编号}-{章节名}-v{版本号}`
3. 按需创建，不要提前建空文档

### 汇报规范
1. **汇报前必须 ls/read 实际文件**，不要凭旧数据或印象
2. **本地完成、飞书完成、审稿完成必须分开汇报**，不能混报成“已完成”
3. 写完再报，先做完再汇报

### Agent 调用规范
1. **持续写作任务默认用 `sessions_send` 发给 Agent 主 Session**
2. **不要用 `sessions_spawn` 创建临时子代理跑持续写作流程**
3. spawn 只留给一次性隔离任务

### 终章单独验收
1. **第 12 章（终章）必须单独验收**，不能混在批量章节里默认通过
2. 终章必须有具体场景，不能只做抽象总结
3. 终章必须回收前文伏笔和情感线
4. 终章必须校验主题闭环是否统一，不能出现理念打架
5. 终章完成后额外写一次"终章专项验收 checkpoint"
